...
from utils.datasets.ligand import LigandDataset
from utils.chem import write_pkl
import gc
...
if __name__ == '__main__':
    ...
    transform = Compose([
        ...
        edge_sampler,
        contrastive_sampler,
        Protein_ligand_relation()
    ])

    dataset = LigandDataset(config.dataset.path, transform=transform)

    dataset_length = len(dataset)
    valid_indices = range(dataset_length - 500, dataset_length)
    train_indices = range(dataset_length - 500)
    ...
    def evaluate(epoch, verbose=1):
        ...
        if verbose:
            logger.info('Evaluate Epoch %d | Average_Loss %.5f | Single Batch Loss %.6f | Loss(Fron) %.6f | Loss(Pos) %.6f | Loss(Cls) %.6f | Loss(Edge) %.6f | Loss(Real) %.6f | Loss(Fake) %.6f | Loss(Surf) %.6f | Loss (Steric) %.6f' % (
                    epoch, average_loss,  loss.item(), loss_frontier.item(), loss_pos.item(), loss_cls.item(), loss_edge.item(), loss_real.item(), loss_fake.item(), loss_surf.item(), loss_steric.item()
                    ))
        return average_loss
    ...
    def train(model, verbose=1, num_epoches=300):
        ...
        for epoch in range(num_epoches):
            ...
            if verbose:
                logger.info('Training Epoch %d | Average_Loss %.5f | Loss %.6f | Loss(Fron) %.6f | Loss(Pos) %.6f | Loss(Cls) %.6f | Loss(Edge) %.6f | Loss(Real) %.6f | Loss(Fake) %.6f | Loss(Surf) %.6f  | Loss(Steric) %.6f' % (
                        epoch+start_epoch, average_loss , loss.item(), loss_frontier.item(), loss_pos.item(), loss_cls.item(), loss_edge.item(), loss_real.item(), loss_fake.item(), loss_surf.item(), loss_steric.item()
                        ))
